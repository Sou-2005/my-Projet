...
WHERE category='تظاهرات ' AND name LIKE ?");
$stmt->execute(["%$search%"]);

}else{

$stmt=$conn->query("SELECT * FROM cultural_heritage 
WHERE category='تظاهرات '");
}
?>
...
<div>التظاهرات والاستعراضات </div>
...
<form method="GET">
<input type="text" name="search" class="form-control" placeholder="ابحث عن تظاهرة ...">
</form>
