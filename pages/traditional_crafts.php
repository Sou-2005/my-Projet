...
WHERE category='الحرف والصناعات التقليدية ' AND name LIKE ?");
$stmt->execute(["%$search%"]);

}else{

$stmt=$conn->query("SELECT * FROM cultural_heritage 
WHERE category='الحرف والصناعات التقليدية '");
}
?>
...
<head>
<meta charset="UTF-8">
<title>الحرف والصناعات التقليدية</title>
...
<div> الحرف والصناعات التقليدية </div>
...
<form method="GET">
<input type="text" name="search" class="form-control" placeholder="ابحث عن حرفة ...">
</form>
