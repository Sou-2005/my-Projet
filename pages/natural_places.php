...
$stmt=$conn->prepare("SELECT * FROM places WHERE category='طبيعي' AND name LIKE ?");
$stmt->execute(["%$search%"]);
$places=$stmt->fetchAll(PDO::FETCH_ASSOC);
}
else{
$stmt=$conn->prepare("SELECT * FROM places WHERE category='طبيعي'");
$stmt->execute();
$places=$stmt->fetchAll(PDO::FETCH_ASSOC);
}
?>
...
<form method="GET">
<input type="text" name="search" placeholder="ابحث عن مكان طبيعي..." value="<?= $search ?>">
<button class="btn btn-light btn-sm">🔍</button>
</form>

</div>

<h2 class="text-center mb-4">الأماكن الطبيعية</h2>
