...
WHERE category='حلويات تقليدية  ' AND name LIKE ?");
$stmt->execute(["%$search%"]);

}else{

$stmt=$conn->query("SELECT * FROM cultural_heritage 
WHERE category='حلويات تقليدية  '");
}
?>
...
<head>
<meta charset="UTF-8">
<title> الحلويات التقليدية</title>
...
<form method="GET">
<input type="text" name="search" class="form-control" placeholder="ابحث عن حلوى تقليدية...">
</form>
