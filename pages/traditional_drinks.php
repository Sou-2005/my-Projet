...
WHERE category='مشروبات  تقليدية ' AND name LIKE ?");
$stmt->execute(["%$search%"]);

}else{

$stmt=$conn->query("SELECT * FROM cultural_heritage 
WHERE category='مشروبات  تقليدية '");
}
?>
...
<form method="GET">
<input type="text" name="search" class="form-control" placeholder="ابحث عن مشروب تقليدي...">
</form>
